			    ...
			    __u8 *data, int interrupt)
{
	...
	for (n = 0; n < count; n++) {
		...
		if(strcmp(hid->driver->name, "apple") == 0 ) {
			if(value[n] != 0) {
				//printk(KERN_INFO "apple-hid-input-field: value[%d]: %d\n", n, value[n]);
			}
			//printk(KERN_INFO "apple-hid-input-field: usage[%d]->code: %d\n", n, field->usage[n].code);
			//printk(KERN_INFO "apple-hid-input-field: field->value[%d]: %d\n", n, field->value[n]);
		}
		...
		if (!(field->flags & HID_MAIN_ITEM_VARIABLE) &&
		    ...
		    field->usage[value[n] - min].hid == HID_UP_KEYBOARD + 1) {
				if(strcmp(hid->driver->name, "apple") == 0 ) {
					printk(KERN_INFO "apple-hid-input-field EXIT");
				}
				goto exit;
			}
	...
	for (n = 0; n < count; n++) {
		...
		if (field->value[n] >= min && field->value[n] <= max
			...
			&& search(value, field->value[n], count)) {
				if(strcmp(hid->driver->name, "apple") == 0 && value[n] != 0) {
					printk(KERN_INFO "1 min:%d max:%d hid:%d maxusage:%d value:%d search:%d\n", 
					min, max, field->usage[value[n] - min].hid, 
					field->maxusage, value[n], 
					search(value, field->value[n], count));
				}
				hid_process_event(hid, field, &field->usage[field->value[n] - min], 0, interrupt);
			} else {
				if(strcmp(hid->driver->name, "apple") == 0 && value[n] != 0) {
					printk(KERN_INFO "FUCK min:%d max:%d hid:%d maxusage:%d value:%d search:%d\n", 
					min, max, field->usage[value[n] - min].hid, 
					field->maxusage, value[n], 
					search(value, field->value[n], count));
				}
		...
		if (value[n] >= min && value[n] <= max
			...
			&& search(field->value, value[n], count)) {
				if(strcmp(hid->driver->name, "apple") == 0 && value[n] != 0) {
					printk(KERN_INFO "2 min:%d max:%d hid:%d maxusage:%d value:%d search:%d\n", 
					min, max, field->usage[value[n] - min].hid, 
					field->maxusage, value[n], 
					search(field->value, value[n], count));
				}
				...
				if(strcmp(hid->driver->name, "apple") == 0 && value[n] != 0) {
					printk(KERN_INFO "FUCK2 min:%d max:%d hid:%d maxusage:%d value:%d search:%d\n", 
					min, max, field->usage[value[n] - min].hid, 
					field->maxusage, value[n], 
					search(field->value, value[n], count));
				}
